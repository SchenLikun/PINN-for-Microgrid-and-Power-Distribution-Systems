...
class PID():
    def __init__(self, path=None, date=None, index=None, **kwargs):
        ...
        Gp = dde.Variable(self.init_Gp)
        Ti = dde.Variable(self.init_Ti)

        self.variable_list = [Gp, Ti]
        ...
        self.pi_ode = PI_ode
        self.real_value_list = [self.real_Gp, self.real_Ti]
    ...
    def build(self, transform=None, **kwargs):
        ...
        if transform is not None:
            ...
            def output_transform(t, y):
                ...
                # send all the data to the same GPU, accelerate it !!!!
                y2 = linear + factor * torch.Tensor([1, 1]).to(y.device) * y
                y2[0, 1] = y_true[0, 1]
                # y2[0, 3] = y_true[0, 3]
                y2[idx, 1] = y_true[idx, 1]
                # y2[0, 3] = y_true[0, 3]

                return y2.to(y.device)
    ...
    def load_model(self, model_path, load_fnamevar_path=None):
        if load_fnamevar_path is None:
            try:
                ...
                self.init_Gp = Chat[-1, 0]
                self.init_Ti = Chat[-1, 1]
...
if __name__ == "__main__":
    input_data_path = './noisy_output_file_0.02.csv'
